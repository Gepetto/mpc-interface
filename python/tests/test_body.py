...
class BodyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        LIP_outputs = {"DCM_x" : LineCombo({"CoM_x":1, "CoM_dot_x":1/3.5}),
                       "DCM_y" : LineCombo({"CoM_y":1, "CoM_dot_y":1/3.5})}
        
        limit = Constraint("CoM_x", 4)
        limit2 = Constraint("s", 10, arrow = [1, 1], axes = ["_x", "_y"],
                            L = np.eye(9))
...
#        SSbox = Box()
    ...
    def test_qp_prob(self):
        ...
        self.assertEqual(set(self.body.domain.keys()),
                         set(self.body.optim_variables+
                             self.body.given_variables))
        
        self.body.make_preview_matrices()
        self.assertEqual(len(self.body.definitions), len(self.body.PM))
        self.assertEqual(self.body.PM["DCM_y"][0].shape, 
                         self.body.PM["CoM_y"][0].shape)
        
        collector = {var:np.array(list(rang)).reshape([-1, 1]) 
        ...
        self.assertTrue((h == self.limit.bound()).all()) # given doesn't affect this inequality
        ...
        self.assertEqual(Q1.shape, (self.body.optim_len, 
                                    self.body.optim_len))
